...
class SvgCreator:
    """Class for generating an svg -format vector graphics drawing of a shakuhachi sheet music page described as ShakuMusic instance
    ...
    def _draw_text(self, text_insert: str, position: tuple, fill: tuple, font_size: int, style=None):
        ...
        """
        self._svg.add(text.Text(text_insert, insert=position, fill=self._rgb(fill), font_size=str(font_size), style=style))
    ...
    def _draw_note(self, file: str, position: tuple):
        ...
        """
        size = [self._scaler(consts.SHEET_NOTE_SIZE) + consts.EXPORT_NOTE_FONT_SIZE_INCREMENT for i in range(2)]
    ...
    def _draw_texts(self, name, composer):
        ...
        self._draw_text(name, self._scaler(consts.NAME_POSITION), consts.TEXT_COLOR, font_size, style="text-anchor:end")
        self._draw_text(composer, self._scaler(consts.COMPOSER_POSITION), consts.TEXT_COLOR, font_size)
    ...
    def create_svg(self, music: ShakuMusic, grid_included: bool=False):
        """Generates an svg -format vector graphics drawing, based on shakuhachi notation depicted in ShakuMusic instance

        Args:
            music: Shakuhachi sheet music as ShakuMusic instance
            grid_included: True if a musical-measure -grid is to be generated into svg. Defaults to False.
